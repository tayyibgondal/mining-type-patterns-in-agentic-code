"""
AI-Powered Type-Related PR Classification Pipeline for Rust (OpenAI / Gemini)

Two-agent pipeline (classifier + validator) that decides whether a Rust PR is
type-related. Mirrors `llm_type_classifier_csharp.py`; only the prompts and
the file-extension references differ. A PR is accepted only if the classifier
says yes AND the validator approves.
"""
...
class RustTypePRClassifier:
    """LLM-powered Rust type-related PR classifier with validation."""

    def __init__(self, model: str = "gpt-4o-mini", max_concurrent: int = 30,
        ...
        log_progress(
            f"Initializing classifier: provider={provider}, model={model}, "
        ...
        if self.provider == "openai":
            ...
            if not GEMINI_AVAILABLE:
                raise ValueError(
                    "Gemini support not available. pip install google-generativeai"
                )
    ...
    def create_classifier_prompt(self, pr_data: Dict) -> str:
        ...
        commit_messages = self.safe_str_slice(
            pr_data.get('commit_messages'), 800, 'No commit messages'
        )
        patch_text = self.safe_str_slice(pr_data.get('patch_text'), 2000, 'No patches available')

        prompt = f"""You are an expert Rust and software engineering analyst. Your task is to determine if a pull request is related to Rust type-system or type-safety changes, fixes, or improvements.

Analyze the following PR data and determine if it is TYPE-RELATED:
...
- Rust files changed: {pr_data.get('rs_files_changed', 0)}

**Definition of TYPE-RELATED for Rust:**
A PR is type-related if it involves any of:
- Adding, modifying, or removing type annotations or signatures
- Adding/removing/refactoring `unsafe` blocks, functions, traits, or impls
- Changes to lifetimes ('a) or lifetime elision
- Changes to generics, trait bounds, where clauses, or `impl Trait` / `dyn Trait`
- Adding or refactoring traits, type aliases, or associated types
- Changes that affect borrow-checker behavior or ownership
- Replacing `.unwrap()` / `.expect()` with proper Result/Option handling (or vice versa)
- `as` casts being added or refactored to safer conversions (e.g., `TryFrom`, `try_into`)
- Pattern-matching changes that hinge on enum/struct types
- Newtype wrapping/unwrapping, PhantomData, smart-pointer (Arc/Mutex/etc.) wrapping changes
- Send/Sync auto-trait fixes
- Fixing Rust compile-time type errors

**Your Task:**
1. Carefully analyze all the provided information.
2. Look for type-related keywords, patterns, and actual type/safety changes.
3. Provide a clear YES or NO decision.
4. Explain your reasoning with specific evidence from the patch.
...
Be thorough and accurate. Only classify as type-related when there is clear evidence."""
    ...
    def create_validator_prompt(self, pr_data: Dict, classification: Dict) -> str:
        ...
        reasoning = self.safe_str_slice(
            classification.get('reasoning'), 500, 'No reasoning provided'
        )

        prompt = f"""You are a senior Rust code reviewer validating type-related PR classifications.
...
- Title: {title}
- Description: {body}
- Files changed: {pr_data.get('rs_files_changed', 0)} Rust files
...
4. Is the confidence level appropriate for Rust-specific type-system concepts?
...
Be critical and thorough in your validation."""
    ...
    async def call_api_with_timeout(self, prompt: str, temperature: float = 0.1
                                    ) -> Tuple[Dict, int]:
        try:
            ...
            return {
                ...
                "evidence": [],
                "detected_in": {"title": False, "body": False,
                                "commits": False, "patches": False}
            }, 0

    async def call_api(self, prompt: str, temperature: float = 0.1
                       ) -> Tuple[Dict, int]:
        self.tasks_queued += 1
    ...
    async def _call_openai(self, prompt: str, temperature: float) -> Tuple[Dict, int]:
        response = await self.client.chat.completions.create(
            ...
            messages=[
                {"role": "system",
                 "content": "You are a precise Rust and software engineering expert. "
    ...
    async def _call_gemini(self, prompt: str, temperature: float) -> Tuple[Dict, int]:
        ...
        def _sync_call():
            ...
            full_prompt = (
                "You are a precise Rust and software engineering expert. "
                "Always respond with valid JSON.\n\n"
                f"{prompt}\n\n"
    ...
    async def classify_pr(self, pr_row: pd.Series, pr_index: int, total: int) -> Dict:
        ...
        pr_data = {
            ...
            'rs_files_changed': safe_get('rs_files_changed', 0),
        }

        classifier_prompt = self.create_classifier_prompt(pr_data)
        classification, classifier_tokens = await self.call_api_with_timeout(
            classifier_prompt, temperature=0.1
        )
        if classification.get('error'):
            log_progress(
                f"[{pr_index}/{total}] PR #{pr_number} - Classifier failed", error=True
            )
        ...
        validation, validator_tokens = await self.call_api_with_timeout(
            validator_prompt, temperature=0.1
        )
        if validation.get('error'):
            log_progress(
                f"[{pr_index}/{total}] PR #{pr_number} - Validator failed", error=True
            )

        elapsed = time.time() - start_time
        is_type = (
            classification.get('is_type_related', False) and
            validation.get('validation_result') == 'APPROVED'
        )
        log_progress(
            f"[{pr_index}/{total}] PR #{pr_number} done in {elapsed:.1f}s - "
    ...
    def get_commit_messages(self, pr_id) -> str:
        try:
            if hasattr(self, 'pr_commits_df') and self.pr_commits_df is not None:
    ...
    def load_pr_data(self, csv_file: str):
        ...
        try:
            log_progress("Loading commit data from AIDev...")
            self.pr_commits_df = pd.read_parquet(
    ...
    async def process_batch(self, batch_df: pd.DataFrame, batch_num: int,
        ...
        log_progress(
            f"Completed batch {batch_num}/{total_batches} - {len(results)} successful"
        )
        return results

    async def classify_all_prs_async(self, limit: Optional[int] = None,
                                     output_file: str = 'rust_classified_type_prs.csv'
                                     ) -> pd.DataFrame:
        self.start_time = time.time()
        log_progress("=" * 80)
        log_progress(f"Starting {self.provider.upper()} Rust Type PR Classification")
        log_progress(
            f"Model: {self.model} | Concurrent: {self.max_concurrent} | "
        ...
        for batch_num in range(num_batches):
            ...
            batch_results = await self.process_batch(
                batch_df, batch_num + 1, num_batches, start_idx
            )
            ...
            log_progress(
                f"Progress: {len(all_results)}/{total_prs} "
                f"({100*len(all_results)/total_prs:.1f}%) | "
        ...
        merged_df = self.pr_df.merge(
            results_df, left_on='id', right_on='pr_id', how='left'
        )
        self.export_results(merged_df, output_file)
        return merged_df

    def classify_all_prs(self, limit: Optional[int] = None,
                         output_file: str = 'rust_classified_type_prs.csv'
                         ) -> pd.DataFrame:
    ...
    def export_results(self, df: pd.DataFrame, output_file: str):
        ...
        export_columns = [
            ...
            'patch_text',
            'classifier_is_type_related', 'classifier_confidence',
            'classifier_reasoning', 'classifier_evidence',
            'detected_in_title', 'detected_in_body',
            'detected_in_commits', 'detected_in_patches',
            'validation_result', 'validator_agreed',
            'validator_confidence', 'validator_reasoning',
            'final_is_type_related', 'tokens_used', 'processing_time_seconds',
        ]
    ...
    def export_summary(self, df: pd.DataFrame, output_file: str):
        ...
        errors = (df.get('error') == True).sum() if 'error' in df.columns else 0
        ...
        summary = {
            ...
            'type_related_percentage': float(type_related / total_classified * 100)
                if total_classified > 0 else 0,
            'total_processing_time_seconds': float(time.time() - self.start_time)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='LLM-powered Rust type-related PR classifier')
    parser.add_argument('--input', default='datasets/rust_data/rust_type_prs_all_groups.csv',
                        help='Input CSV file with PRs')
    parser.add_argument('--output', default='datasets/rust_data/rust_classified_type_prs.csv',
    ...
    parser.add_argument('--model', default=None,
                        help='Model (default: gpt-4o-mini for OpenAI, '
    ...
    classifier = RustTypePRClassifier(
        model=args.model,
    ...
    classifier.classify_all_prs(limit=args.limit, output_file=args.output)
